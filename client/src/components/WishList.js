...
const WishList = (props) => {
    ...
    async function removeLike(rid) {
        const { data } = await axios.patch(`http://localhost:4000/users/unlike?uid=${props.user._id}&rid=${rid}`);
        setDeleteData(data);
    };
    ...
    useEffect(
        () => {
            ...
            async function fetchData() {
                try {
                    const recipe = await axios.get(`http://localhost:4000/users/wishlist/${props.user._id}`);
